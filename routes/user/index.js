...
router.post('/login', function(req, res, next){
  User.findOne({username: req.body.username}).select('email username password active permission').exec(function(err, user){
    if (err) throw err
    ...
    if (!user) {
      ...
      if (!validPassword) {
        ...
        var token = jwt.sign({username: user.username, email: user.email, permission: user.permission}, secret, { expiresIn: '14d' })
